...
const addNewTransaction = async (transaction) => {
    try {
        const { user_id, transaction_type, amount, item_name, transaction_date } = transaction

        const newTransaction = await db.one(`INSERT INTO transactions (user_id, transaction_type, amount, item_name, transaction_date ) VALUES($1, $2, $3, $4, $5) RETURNING *`, [user_id, transaction_type, amount, item_name, transaction_date ])
